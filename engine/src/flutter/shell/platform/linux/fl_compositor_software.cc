...
static gboolean fl_compositor_software_present_layers_impl(
...
static void fl_compositor_software_get_frame_size_impl(FlCompositor* compositor,
                                                       size_t* width,
                                                       size_t* height) {
  FlCompositorSoftware* self = FL_COMPOSITOR_SOFTWARE(compositor);
...
static gboolean fl_compositor_software_render_impl(FlCompositor* compositor,
                                                   cairo_t* cr,
  ...
  const double scale =
      surface == nullptr ? 1.0 : fl_gtk_surface_get_scale(surface);
  if (wait_for_frame) {
    gint64 expiry_time =
...
static void fl_compositor_software_class_init(
  ...
  FL_COMPOSITOR_CLASS(klass)->present_layers =
      fl_compositor_software_present_layers_impl;
  FL_COMPOSITOR_CLASS(klass)->get_frame_size =
      fl_compositor_software_get_frame_size_impl;
  FL_COMPOSITOR_CLASS(klass)->render = fl_compositor_software_render_impl;
#if FLUTTER_LINUX_GTK4
...
FlCompositorSoftware* fl_compositor_software_new() {
  ...
  return fl_compositor_software_new(task_runner);
}
...
gboolean fl_compositor_software_composite_layers(
    ...
    const FlutterLayer** layers,
    size_t layers_count) {
  if (!fl_compositor_present_layers(FL_COMPOSITOR(compositor), layers,
                                    layers_count)) {
    return FALSE;
  }
  return fl_compositor_software_render_impl(FL_COMPOSITOR(compositor), cr,
                                            nullptr, FALSE);
}
#endif
