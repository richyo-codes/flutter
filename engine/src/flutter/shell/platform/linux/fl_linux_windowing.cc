...
static GtkWindow* create_toplevel_window() {
  GApplication* application = g_application_get_default();
  if (GTK_IS_APPLICATION(application)) {
    // Associate secondary windows with the running FlApplication. Besides
    // matching GTK's application lifecycle, this lets GTK register their
    // toplevel surfaces with the desktop shell.
    return GTK_WINDOW(gtk_application_window_new(GTK_APPLICATION(application)));
  }
