...
static GtkAccessible* fl_render_texture_gtk4_get_first_accessible_child(
    GtkAccessible* accessible) {
  FlRenderTextureGtk4* self = FL_RENDER_TEXTURE_GTK4(accessible);
  return self->accessible_child == nullptr
             ? nullptr
             : GTK_ACCESSIBLE(g_object_ref(self->accessible_child));
}

static void fl_render_texture_gtk4_accessible_iface_init(
  ...
  iface_4_10->get_first_accessible_child =
      fl_render_texture_gtk4_get_first_accessible_child;
}
...
G_DEFINE_TYPE_WITH_CODE(
    ...
    G_IMPLEMENT_INTERFACE(GTK_TYPE_ACCESSIBLE,
                          fl_render_texture_gtk4_accessible_iface_init))
...
static void fl_render_texture_gtk4_measure(GtkWidget* widget,
  ...
  const int texture_size = orientation == GTK_ORIENTATION_HORIZONTAL
                               ? self->texture_width
                               : self->texture_height;
  const int scale_factor = MAX(gtk_widget_get_scale_factor(widget), 1);
  const int natural_size = (texture_size + scale_factor - 1) / scale_factor;

  // The Flutter view fills its GTK allocation. Its backing texture must not
  // become a minimum size, otherwise a large frame can prevent the window
  // from shrinking. Sized-to-content views set an explicit size request.
  *minimum = 0;
...
void fl_render_texture_gtk4_set_texture(FlRenderTextureGtk4* self,
  ...
  if (texture_size_changed) {
    flutter_linux_gtk4_dbg(
        "render_texture_size_changed",
        "self=%p old_texture_width=%d old_texture_height=%d "
        "new_texture_width=%d new_texture_height=%d texture=%p",
        self, old_texture_width, old_texture_height, new_texture_width,
        new_texture_height, texture);
    fl_render_texture_gtk4_update_accessible_name(self);
...
void fl_render_texture_gtk4_set_accessible_child(
    FlRenderTextureGtk4* self,
    GtkAccessible* accessible_child) {
  g_return_if_fail(FL_IS_RENDER_TEXTURE_GTK4(self));
  ...
  g_set_object(&self->accessible_child, accessible_child);
}
