...
void fl_scrolling_manager_handle_scroll_event(FlScrollingManager* self,
                                              ...
                                              gdouble scroll_delta_x,
                                              gdouble scroll_delta_y,
                                              gint scale_factor) {
  g_return_if_fail(FL_IS_SCROLLING_MANAGER(self));
  ...
  if (source_device != nullptr &&
    ...
    if (gdk_event_is_scroll_stop_event(event)) {
      ...
      fl_engine_send_pointer_pan_zoom_event(
          engine, self->view_id, event_time * kMicrosecondsPerMillisecond,
          self->last_x, self->last_y, kPanZoomUpdate, self->pan_x, self->pan_y,
          1, 0);
    }
  } else {
    fl_engine_send_mouse_pointer_event(
        ...
        kFlutterPointerDeviceKindMouse, scroll_delta_x, scroll_delta_y, 0, 0,
        0);
  }
}
